...
import { Mail } from 'lucide-react'; // Using a generic mail icon
...
export default function SignInPage() {
  ...
  useEffect(() => {
    ...
    // If user (email) becomes non-null while on this page,
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      // On success, user state in AuthContext will update,
      // and useEffect in AuthContext or this page will handle redirection.
      // Typically to /about-us first as per original user flow, then dashboard
      // AuthContext redirects to '/' if user is set and on '/signin'
    } catch (error) {
      console.error("Sign-in attempt from SignInPage failed:", error);
      toast({
        title: "Error",
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-background to-primary/10 p-6">
      <div className="w-full max-w-md text-center">
        ...
        <div className="bg-card p-8 rounded-xl shadow-2xl space-y-6">
          <h2 className="text-2xl font-semibold text-card-foreground">Enter the App</h2>
          ...
          <p className="text-xs text-muted-foreground mt-6">
            By proceeding, you acknowledge this is a simplified access method for demo purposes and agree to our <Link href="/terms-and-conditions" className="underline hover:text-primary">Terms of Service</Link>.
          </p>
        </div>
